 ...
 */
case class Plan(
  id: Long, summary: String, details: Option[String],
...
object Plan extends SQLSyntaxSupport[Plan] {
  ...
  def apply(p: ResultName[Plan], creator: User, offerer: Option[User])(rs: WrappedResultSet): Plan = Plan(
    id = rs.long(p.id),
    summary = rs.string(p.summary),
    details = rs.stringOpt(p.details),
    creatorId = rs.long(p.creatorId),
    creator = creator,
    offererId = rs.longOpt(p.offererId),
    offerer = offerer
  )
  ...
  def find(id: Long)(implicit session: DBSession = AutoSession): Option[Plan] = withSQL {
    select.from(Plan as p)
      .join(User as c).on(p.creatorId, c.id)
      .leftJoin(User as o).on(p.offererId, o.id)
      .where.eq(p.id, id)
  ...
  def findMatched(limit: Option[Int] = None)(implicit session: DBSession = AutoSession): List[Plan] = withSQL {
    withLimit(limit) {
      select.from(Plan as p)
        .join(User as c).on(p.creatorId, c.id)
        .join(User as o).on(p.offererId, o.id)
        .orderBy(p.id).desc
    }
  ...
  def setOfferer(plan: Plan, offerer: User)(implicit session: DBSession = AutoSession): Plan = {
    withSQL {
      update(Plan as p).set(p.offererId -> offerer.id).where.eq(p.id, plan.id)
    }.update.apply()
